...
method: 提出一种结合历史时间信息与帧间关联机制的数据驱动方法，通过挖掘时序相关性来捕捉目标的运动特征。
result: 在由PIPER X机械臂组成的地面实验平台上，成功实现了对模拟微重力环境下动态目标的平稳且鲁棒的操作。
conclusion: 研究表明，利用帧间关联机制能显著提升空间机器人在未知动态场景下生成平滑操作轨迹的能力。
---
