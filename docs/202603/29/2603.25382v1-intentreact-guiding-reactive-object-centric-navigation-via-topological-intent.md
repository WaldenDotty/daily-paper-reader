...
motivation: 现有的拓扑导航方法在局部决策时仅依赖当前视野，导致在局部观测具有误导性时无法有效利用全局拓扑信息进行纠偏。
method: 提出一种意图条件化的导航框架，通过将全局拓扑引导转化为低维方向信号来偏置局部航点预测，实现拓扑一致的运动。
result: 实验结果表明，IntentReact 在导航成功率和执行质量方面均显著优于之前的对象中心导航基准方法。
conclusion: 在全局规划与局部控制间引入拓扑意图接口，能有效克服局部感知的局限性，提升机器人在复杂环境下的导航性能。
---
