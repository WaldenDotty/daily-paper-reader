...
score: 7.0
evidence: 地外导航与路径规划
tldr: 针对地外探测器自主导航中缺乏高效全局地图的问题，本文提出利用神经辐射场（NeRF）构建可高效存储的3D场景表示。通过结合NeRF提取的地形特征与核脊回归，该框架能将局部观测插值到全局区域，实现动态避障与路径重规划。仿真实验证明，该方法在降低路径成本和提高任务成功率方面优于现有基准，为深空探测提供了新的导航思路。
selection_source: fresh_fetch
motivation: 现有的地外探测器自主导航受限于缺乏易于构建、存储且支持在线重规划的全局地图。
method: 提出一种基于NeRF的地图构建框架，利用核脊回归在NeRF提取的地形特征上对局部成本进行全局插值。
result: 在高保真仿真环境中，该方法相比多种基准方案实现了更低的路径成本和更高的导航成功率。
conclusion: NeRF作为一种高效的3D场景表示，能够有效整合局部与全局信息，显著提升地外探测器的自主路径规划能力。
---
