...
tldr: 针对黑箱系统或纯数据驱动系统的降阶建模（ROM）中存在的预测精度低和不稳定性问题，本文提出了GasNiTROM框架。该框架通过非侵入式方法，结合基于Lyapunov的参数化潜空间动力学和矩阵流形参数化的斜投影算子，在保证全局渐近稳定性的同时，有效捕捉系统的敏感机制。实验证明，该方法在精度和稳定性上均优于现有技术。
selection_source: fresh_fetch
motivation: 现有的非侵入式降阶模型在处理远离平衡态的大瞬态系统时，常因正交投影截断重要状态而导致预测精度差且易失稳。
method: 提出一种结合Lyapunov参数化潜空间动力学与斜投影算子优化的非侵入式框架，并利用伴随反向传播推导闭式梯度。
result: 在三维ODE系统和高雷诺数驱动腔流实验中，该模型不仅实现了全局渐近稳定，且预测精度显著提升。
conclusion: GasNiTROM通过同时优化稳定性和投影算子，为复杂动力系统的非侵入式降阶建模提供了一种高效且稳健的解决方案。
---

## 摘要
对于使用黑箱求解器模拟或仅从数据中获知的系统，非侵入式降阶建模技术是必不可少的。对于表现出剧烈瞬态且运行在远离平衡态的系统，当前的非侵入式模型往往预测精度较差，甚至在无限或有限时间内表现出不稳定性。最近的进展通过在简化哈密顿或拉格朗日全阶动力学时寻求保结构的潜空间架构，或者通过在潜空间中利用 Lyapunov 信息参数化来强制执行全局稳定性，从而解决了稳定性问题。然而，这些进展并不一定能提高所得模型的预测精度，因为这些公式使用正交投影实现降维，而正交投影可能会意外地截断具有重要动力学特征的状态。在本文中，我们通过引入一个非侵入式框架来同时解决这两个问题，该框架旨在同时识别全局渐近稳定的潜空间动力学，以及能够捕捉系统敏感性机制的斜投影算子。具体而言，给定潜空间张量的基于 Lyapunov 的参数化以及斜投影算子的矩阵流形参数化，我们针对高保真训练轨迹拟合模型。此外，我们证明了目标函数的梯度可以使用潜空间中基于伴随的反向传播以闭式形式写出，从而消除了对自动微分的需求。我们在三维常微分方程系统和雷诺数 Re=8300 的二维顶盖驱动流上，将我们的公式与最先进的方法进行了比较。我们证明了我们的模型不仅是全局渐近稳定的（正如构造所预期的那样），而且精度也显著更高。
