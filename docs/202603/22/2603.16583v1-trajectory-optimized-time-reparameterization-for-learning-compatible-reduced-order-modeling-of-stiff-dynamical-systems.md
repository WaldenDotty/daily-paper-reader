...
score: 6.0
evidence: 针对刚性动力学系统的轨迹优化时间再参数化
tldr: 针对刚性动力系统在机器学习降阶模型（ML-ROM）中显式积分不稳定、隐式积分开销大的挑战，本文提出了一种轨迹优化时间再参数化（TOTR）方法。该方法将时间变换建模为弧长坐标下的优化问题，通过惩罚拉伸时间内的加速度来平滑训练动力学。在多个刚性系统上的实验表明，TOTR显著提升了模型的学习效率，相比现有方法将预测误差降低了1-2个数量级，为多尺度动力学系统提供了稳健的显式建模方案。
selection_source: fresh_fetch
motivation: 旨在解决刚性动力系统在机器学习降阶模型中因显式积分不稳定或隐式积分计算昂贵而导致的训练困难问题。
method: 提出TOTR方法，通过在弧长坐标下优化遍历速度剖面以最小化拉伸时间内的加速度，从而生成更易于学习的平滑轨迹。
result: 在参数化线性系统、van der Pol振子和HIRES化学动力学模型中，该方法比现有时间再参数化技术实现了更低的预测误差和更平滑的参数化效果。
conclusion: 研究证明有效缓解ML-ROM刚性的关键在于时间映射的正则性和可学习性，而基于优化的再参数化是处理多尺度系统的有力工具。
---
