...
## 摘要
在动态、非结构化环境中运行的机器人必须在感知受限的情况下平衡安全性和效率。虽然控制障碍函数 (CBF) 通过安全滤波提供了一种原则性的避障方法，但其行为通常由固定参数决定，这可能导致在良性场景中过于保守，或在危险附近过于激进。我们提出了 AlphaAdj，这是一个视觉到控制的导航框架，它利用第一视角 RGB 输入实时调整 CBF 安全滤波器的保守程度。视觉语言模型 (VLM) 根据当前摄像机视图生成一个有界的标量风险评估值，我们将其映射以动态更新 CBF 参数，从而调节安全约束的执行强度。为了解决实际应用中的异步推理和显著的 VLM 延迟问题，我们将几何速度感知动态上限和陈旧度门控融合策略与轻量级实现方案相结合，从而降低了端到端推理开销。我们在多种环境下的多个静态和动态障碍物场景中评估了 AlphaAdj，并与固定参数和无上限的消融实验进行了对比。结果表明，AlphaAdj 在保持无碰撞导航的同时，相对于固定设置将效率（路径长度和到达目标的时间）提高了多达 18.5%，并且相对于无上限基准提高了鲁棒性和成功率。
