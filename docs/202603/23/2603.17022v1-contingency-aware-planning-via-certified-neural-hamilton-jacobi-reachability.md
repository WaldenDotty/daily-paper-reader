...
## 摘要
Hamilton-Jacobi (HJ) 可达性为动力系统提供了形式化的安全保证，但求解高维 HJ 偏微分方程限制了其在实时规划中的应用。本文提出了一种应急感知的多目标导航框架，该框架在未知环境中集成了基于学习的可达性与基于采样的规划。我们使用傅里叶神经算子 (FNO) 来近似不同障碍物配置下 Hamilton-Jacobi-Isaacs 变分不等式的解算子。我们首先为安全后向到达-规避集提供了理论上的下近似保证，从而实现了对所学可达集的形式化安全认证。随后，我们将经认证的可达集与增量式多目标规划器集成，该规划器强制执行可达集约束和一种保证在有限时间内返回安全区域的恢复策略。总而言之，我们证明了所提出的框架实现了具有可证明应急行为的渐近最优导航，并通过在 Webots 仿真中对 KUKA youBot 的实时部署验证了其性能。
