...
title_zh: COMPASS：通过代理令牌与共享空间实现泛在感知的完备多模态融合
...
## 摘要
模态缺失仍然是多模态感知面临的主要挑战，因为大多数现有方法通过丢弃缺失分支、使用特定子集的融合或重建缺失特征，使融合过程适应观测到的子集。结果，融合头接收到的输入结构往往与训练时不同，导致融合不完整以及跨模态交互性能下降。我们提出了 COMPASS，这是一个基于融合完备性原则构建的缺失模态融合框架：融合头始终接收固定的 N 插槽多模态输入，每个模态插槽对应一个令牌。对于每个缺失模态，COMPASS 在共享潜空间中使用成对的源到目标生成器，从观测模态中合成特定目标的代理令牌，并将其聚合为单个替换令牌。为了使这些代理既具备表示兼容性又包含任务信息，我们结合了代理对齐、共享空间正则化以及针对每个代理的判别性监督。在 XRF55、MM-Fi 和 OctoNet 数据集上进行的多种单模态和多模态缺失设置下的实验表明，COMPASS 在绝大多数场景中均优于现有方法。我们的结果表明，保持模态完备的融合接口是实现鲁棒多模态感知的一种简单且有效的设计原则。
