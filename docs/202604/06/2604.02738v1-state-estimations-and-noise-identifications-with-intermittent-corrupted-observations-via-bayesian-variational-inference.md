...
score: 7.0
evidence: 传感器网络中存在观测干扰时的状态估计与噪声识别
tldr: 针对分布式传感器网络中存在的数据包丢失、观测污染及噪声协方差未知等复杂问题，本文提出了一种基于贝叶斯变分推理的自适应卡尔曼滤波器（VB-AKF）。该方法通过引入双掩码生成模型，利用两个独立的伯努利随机变量显式刻画通信丢失与数据真实性，并整合多传感器观测信息。实验证明，该方法在状态估计和参数识别方面具有渐近最优性，性能随传感器数量增加趋近理论下界。
selection_source: fresh_fetch
motivation: 解决分布式传感器网络中数据包丢失、观测异常值和未知噪声参数共存时的状态估计难题。
method: 提出一种VB-AKF算法，采用双掩码生成模型区分通信损耗与数据可靠性，并利用变分推理近似联合后验概率。
result: 数值实验验证了该方法在处理间歇性损坏观测时的有效性，且估计精度随传感器规模增加而显著提升。
conclusion: 该研究为复杂网络环境下的状态估计提供了鲁棒且渐近最优的解决方案，实现了参数识别与状态估计的协同优化。
---

## 摘要
本文关注分布式传感器网络中的状态估计问题，其中间歇性数据包丢失、受损观测以及未知噪声协方差并存。为了应对这一挑战，我们将系统状态、噪声参数和网络可靠性的联合估计建模为一个贝叶斯变分推断问题，并提出了一种新型变分贝叶斯自适应卡尔曼滤波器（VB-AKF），用于近似潜在参数的联合后验概率密度。与现有的分别处理缺失数据和测量离群值的 AKF 不同，所提出的 VB-AKF 采用了一种包含两个独立伯努利随机变量的双掩码生成模型，显式地刻画了可观测的通信丢失和潜在的数据真实性。此外，VB-AKF 将多个并发观测集成到自适应滤波框架中，显著增强了统计可辨识性。综合数值实验验证了所提方法的有效性和渐近最优性，结果表明，随着传感器数量的增加，参数辨识和状态估计均渐近收敛于理论最优下界。
