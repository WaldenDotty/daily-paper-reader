...
tldr: 本综述全面回顾了无人机视觉语言导航（Aerial VLN）领域，重点探讨了大语言模型（LLM）和视觉语言模型（VLM）的集成。文章定义了单指令和对话式两种交互范式，将现有方法归纳为五大架构类别，并系统分析了评估基础设施及性能权衡。最后，针对长程指令理解、6自由度动作执行及实机部署等七大开放性问题提出了未来研究方向。
selection_source: fresh_fetch
motivation: 旨在解决无人机在复杂三维环境中理解自然语言指令并实现自主导航的挑战，特别是探索大模型时代下的新技术路径。
method: 提出了一种涵盖序列到序列、端到端大模型、分层架构、多智能体及对话式导航五大类别的分类法，并对设计逻辑进行深度剖析。
result: 总结了现有数据集、仿真平台和评价指标的局限性，并对比了不同架构在离散与连续动作空间及仿真到现实迁移中的表现。
conclusion: 指出了长程指令对齐、机载部署及多机协同等七个关键研究方向，为推动无人机视觉语言导航技术的发展提供了路线图。
---
