...
## 摘要
机器人系统的数据驱动安全性验证通常依赖于区带对称体（zonotopic）可达性分析，因为它具有良好的可扩展性和计算效率。然而，对于非线性系统，这些方法往往过于保守，尤其是在长预测时界和存在测量噪声的情况下。我们提出了一种基于 Koopman 算子和区带对称体集合表示的数据驱动可达性框架，该框架将非线性系统提升到有限维的线性状态-输入相关模型中。随后在提升空间中计算可达集，并将其投影回原始状态空间，从而获得真实动力学的保证超近似（over-approximations）。所提出的方法在保留形式化安全保证的同时降低了保守性，并且我们证明了所得的可达集是对真实可达集的超近似。在自动驾驶车辆上的数值模拟和真实世界实验表明，与基于模型的方法和线性数据驱动方法相比，该方法能够产生显著更紧致的可达集超近似，特别是在长时界预测中。
