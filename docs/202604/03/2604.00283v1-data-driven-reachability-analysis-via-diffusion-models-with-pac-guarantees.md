...
## 摘要
我们提出了一种用于非线性动力系统可达性分析的数据驱动框架，该框架不需要显式模型。去噪扩散概率模型仅从轨迹数据中学习动力系统随时间演化的状态分布。预测的可达集表现为基于重构误差导出的非一致性得分的次水平集形式，其阈值通过“先学习后测试”（Learn Then Test）程序进行校准，以确保排除可达状态的概率在高概率下是有界的。在受迫 Duffing 振子、平面四旋翼飞行器以及高维反应扩散系统这三个非线性系统上的实验证实，经验漏检率保持在概率近似正确（PAC）界限以下，同时该方法能够扩展到经典基于网格和多项式方法无法处理的高维状态空间。
