...
score: 6.0
evidence: 自主导航的多目标规划
tldr: 针对自动驾驶导航中多目标优化问题，传统加权和方法无法找到非凸区域的帕累托最优解。本文提出一种基于大邻域搜索（LNS）的搜索算法，专门解决加权最大化规划问题。该方法在保持解质量的同时，比现有加权最大化规划器提速1-2个数量级，显著提升了在离散领域寻找复杂权衡方案的效率，使其在实际导航任务中更具可行性。
selection_source: fresh_fetch
motivation: 传统的加权和方法无法捕捉多目标优化中非凸区域的帕累托最优解，而现有的加权最大化方法在离散领域计算复杂度过高。
method: 提出一种基于大邻域搜索（LNS）框架的新型搜索算法，旨在高效求解加权最大化规划问题。
result: 仿真实验证明，该算法在解质量相当的前提下，运行速度比现有加权最大化规划器提升了1到2个数量级。
conclusion: 该研究为自动驾驶提供了一种高效的多目标规划方案，成功克服了加权最大化方法的计算瓶颈。
---
