...
tldr: 本文针对受局部未知输入影响的大规模离散时间系统，提出了一种新型分布式未知输入观测器（DUIO）框架。该框架允许各节点先基于局部测量估计最大可重构状态，再通过分布式优化算法协同融合各节点信息以重构全局状态。研究不仅证明了估计误差的有界性，还引入归一化步骤以应对系统几何条件不佳的问题，为分布式状态估计提供了高效且鲁棒的解决方案。
selection_source: fresh_fetch
motivation: 旨在解决大规模分布式传感器网络在存在局部未知输入干扰时，如何准确重构系统全局状态的挑战。
method: 提出一种结合局部状态重构与分布式优化融合的两阶段框架，并嵌入归一化步骤以优化收敛性能。
result: 理论分析给出了显式的估计误差界限，仿真结果验证了该方法在处理病态系统几何结构时的有效性与性能提升。
conclusion: 该基于优化的分布式观测器框架能够有效融合局部信息，在未知输入环境下实现高性能的大规模系统状态估计。
---

## 摘要
本文提出了一种新型分布式未知输入观测器（DUIO）框架，用于受局部未知输入影响的大规模系统的状态估计。我们考虑这样一类系统：其输出由空间分布的传感器网络测量，而输入通过多个分散通道引入。在该框架下，每个局部节点仅利用其本地输入和输出测量值来估计最大局部可重构状态。随后，各节点通过融合这些部分估计值的分布式优化算法，协作重构整个系统状态。我们提供了严谨的分析，证明估计误差是有界的，且误差界限显式地取决于每个时间步的通信迭代次数以及由系统参数决定的强凸性常数。此外，为了抵消由不良系统几何条件引起的曲率各向异性，我们在分布式优化过程中嵌入了一个归一化步骤。仿真结果证明了所提框架的有效性以及归一化步骤带来的性能提升。
