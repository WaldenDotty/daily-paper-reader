...
## 摘要
多输出高斯过程为向量值场的学习提供了具有原则性的不确定性感知能力，但由于其计算复杂性和集中式特性，难以部署在大规模、分布式和流式场景中。本文提出了一种基于共识的递归多输出高斯过程（CRMGP）框架，该框架将共享基向量上的递归推理与邻居间的信息共识更新相结合。所提出的方法支持并行、完全分布式的学习，且单步计算量有界，同时保留了输出间的相关性和校准后的不确定性。在合成风场和真实 LiDAR 数据上的实验表明，CRMGP 实现了具有竞争力的预测性能和可靠的不确定性校准，为多智能体感知应用提供了一种可扩展的方案，可作为集中式高斯过程模型的替代。
