...
date: 2026-04-21
pdf: "https://arxiv.org/pdf/2604.19160v1"
tags: ["query:cislunar-dyn-nav"]
score: 6.0
evidence: 动态环境下的多传感器融合多目标跟踪
tldr: 本研究针对分布式传感器网络中的多目标跟踪问题，提出了一种基于多智能体坐标下降的控制方法。通过引入自适应互补融合机制，优先处理高信息量传感器的数据，实现了传感器动作的分布式共识。该方法在保证计算效率的同时，显著提升了大规模实时感知系统的跟踪精度和扩展性，有效解决了资源受限环境下的多目标状态估计难题。
selection_source: fresh_fetch
motivation: 旨在解决分布式移动传感器网络在多目标跟踪中面临的资源协调、计算负担及跟踪精度平衡问题。
method: 提出一种结合多智能体坐标下降与自适应互补融合的控制方法，通过优先考虑高信息量传感器来实现分布式动作共识。
result: 实验表明，该方法在多种挑战性场景下显著提升了多目标跟踪的精度，并大幅优化了计算效率。
conclusion: 该分布式控制框架为大规模实时感知系统提供了一种兼具扩展性、灵活性与高效性的多目标跟踪解决方案。
---

## 摘要
在动态环境中使用分布式传感器网络跟踪多个目标是统计信号处理中的一个基本问题。在此类场景中，移动传感器网络必须协调其动作，以准确估计多个目标的位置和轨迹，并在有限的计算和通信资源与多目标跟踪精度之间取得平衡。多传感器控制方法可以通过实现资源的有效利用并提高估计目标状态的准确性来提升这些网络的性能。本文提出了一种利用多智能体坐标下降法解决该问题的新型多传感器控制方法，确保了整个传感器网络中最佳传感器动作的分布式共识。为此，本文开发了一种新型自适应互补融合方法，优先处理来自信息量最大的传感器的信息。我们的方法提高了计算的可处理性并实现了完全分布式控制，确保了大规模实时传感系统所需的扩展性和灵活性。在多个具有挑战性的多目标跟踪场景下的实验结果表明，与现有竞争方法相比，我们的方法显著提高了多目标跟踪精度和计算效率。
