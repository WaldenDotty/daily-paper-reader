...
## 摘要
本文研究了在自主堆栈为黑盒的情况下，自主机器人执行持续监视任务时的运行时监控。环境被划分为有限个部分，每个部分都带有一个不确定性状态，该状态在被观察时减小，否则增加。我们将闭环系统建模为具有线性参数变化动力学的状态依赖混合系统，并设计了一个基于离线计算的不变性的监控器。由于对于大型待监视空间，这种不变性通常难以获得，我们提出了一种组合式监控器，通过对每个不确定性区域进行低维不变集的去中心化计算，并在在线检查它们的合取来获得。在常见的独立性假设下，该组合式监控器相对于全系统不变性是可靠且完备的。该方法应用于一个真实机器人持续监视迷宫的案例研究中，强调了其在实践中的适用性。
