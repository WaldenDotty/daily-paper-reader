...
motivation: 旨在解决从轨迹数据中学习具有物理一致性且具备稳定保证的复杂动力系统控制律的挑战。
method: 采用交替优化策略协同学习pH系统模型和EB-PBC控制器，并利用神经网络嵌入物理结构和耗散正则化。
result: 在平面和扭转单摆系统的状态调节及起摆任务中，成功实现了具有鲁棒性和稳定性的控制效果。
conclusion: 该框架通过结合物理先验与深度学习，为实现可解释、稳定且高效的非线性系统能量控制提供了有效途径。
---
