...
title_zh: 面向低轨巨型星座的可扩展 SDN：一种图学习方法
...
score: 6.0
evidence: 利用Koopman理论对非线性卫星星座动力学进行线性化
tldr: 针对低轨（LEO）巨型星座网络管理的扩展性瓶颈，本文提出一种分层软件定义网络（SDN）框架。该框架结合图神经网络（GNN）与库普曼（Koopman）理论，通过图库普曼自动编码器（GKAE）对星座拓扑进行时空预测。实验表明，该方法在Starlink星座上显著提升了空间压缩和时间预测性能，且模型更轻量，为大规模卫星网络的高效管理提供了新方案。
selection_source: fresh_fetch
motivation: 随着低轨卫星星座规模达到数千颗，传统的网络管理方式在处理复杂的动态拓扑时面临严重的扩展性瓶颈。
method: 提出一种分层SDN架构，利用图库普曼自动编码器（GKAE）将非线性星座动力学线性化，实现高效的时空行为预测。
result: "在Starlink星座仿真中，该方法相比基准模型在空间压缩上提升了42.8%，时间预测精度提升了10.81%，且模型体积更小。"
conclusion: 该研究证明了结合图学习与算子理论能有效解决巨型星座的扩展性挑战，为未来非地面网络（NTN）的全球协调控制奠定了基础。
---

## 摘要
地面网络的局限性推动了非地面网络（NTN）的整合，特别是包含数千颗低地球轨道（LEO）卫星的巨型星座。虽然这些卫星通过星间链路（ISL）充当互连的网络交换机，但其庞大的规模给网络管理带来了严重的瓶颈。为解决这一问题，我们提出了一种可扩展的分层软件定义网络（SDN）框架。我们的架构利用图神经网络（GNN）来紧凑地表示星座拓扑，并利用 Koopman 理论将非线性动力学线性化。具体而言，图 Koopman 自动编码器（GKAE）在每个轨道壳层的线性子空间内预测时空行为。随后，中央 SDN 控制器汇总这些壳层级预测，以实现全球协调控制。在 Starlink 星座上的仿真表明，与现有基准相比，我们的方法在空间压缩方面至少提升了 42.8%，在时间预测方面提升了 10.81%，同时显著减小了模型占用空间。
