...
score: 6.0
evidence: 带终端代价和不确定性的动力学规划
tldr: 针对机器人运动规划中忽略终点状态质量的问题，本文提出了一种包含终端代价的动力学约束规划框架KiTe。该方法将终端状态的偏好和可靠性纳入优化目标，并扩展至信念空间，通过最小化终端信念与目标间的Wasserstein距离来提高到达成功率。KiTe支持从数据中学习动力学和不确定性模型，在多个仿真和真实实验中显著提升了不确定环境下的任务成功率。
selection_source: fresh_fetch
motivation: 现有的动力学约束规划器通常只优化累积轨迹代价，将到达目标视为可行性检查，而忽略了终点状态的质量和可靠性。
method: 提出了一种包含终端代价的AO-RRT扩展算法KiTe，通过在信念空间中最小化Wasserstein距离来优化目标到达概率，并支持学习动力学模型。
result: 在Flappy Bird、自动泊车和平面推动物体等实验中，KiTe在不确定性环境下的一致性表现优于现有方法，并成功应用于真实机器人。
conclusion: 引入终端代价并结合学习的信念动力学，能有效提升机器人运动规划在复杂不确定环境下的鲁棒性和目标达成率。
---
