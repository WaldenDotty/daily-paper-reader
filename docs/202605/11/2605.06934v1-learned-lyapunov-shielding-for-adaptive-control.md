...
title_zh: 面向自适应控制的学习型 Lyapunov 屏蔽
authors: "Giansalvo Cirrincione, Adriano Fagiolini"
date: 2026-05-07
pdf: "https://arxiv.org/pdf/2605.06934v1"
tags: ["query:cislunar-dyn-nav"]
score: 6.0
evidence: 自适应控制中用于稳定性和安全的李雅普诺夫屏蔽
tldr: 本研究针对Euler-Lagrange系统，通过结合学习到的结构化Lyapunov函数、残差Soft Actor-Critic策略和物理信息神经网络（PINN），增强了传统的Slotine-Li自适应控制器。提出了一种闭式安全滤波器，无需在线QP求解即可确保系统稳定性。理论证明了其全局可行性与指数稳定性，并在2自由度和7自由度机械臂上验证了显著降低跟踪误差的效果。
...
## 摘要
我们通过三个学习组件增强了用于 Euler-Lagrange 系统的 Slotine-Li 自适应控制器：一个结构化二次 Lyapunov 函数 $V_ψ$，其正定性由 Cholesky 参数化保证；一个残差 Soft Actor-Critic 策略，用于向解析基准添加有界转矩修正；以及一个用于估计未建模动力学的物理信息神经网络 (PINN)。基于单个仿射约束 $\dot V_ψ + αV_ψ \le 0$ 推导出的闭式安全过滤器，可将每个策略输出投影到安全集上
