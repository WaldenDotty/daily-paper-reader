...
## 摘要
从离散观测中恢复连续时间动力学具有挑战性，因为随着观测间隔的增加，局部监督（如逐点回归目标、导数近似或方程残差）的保真度会下降。我们将局部监督替换为一种全局结构约束：任何表示自治动力学的流在时间平移下必须满足半群性质。我们训练了一个以时间为条件的割线速度场，其对该性质的偏离（我们称之为“对称性破裂”）具有两个用途。作为训练正则化项，它将假设空间限制在跨时间尺度一致组合的流中。作为推理预言机，它允许求解器选择保持内部一致性的最大步长，从而取代了传统自适应求解器所依赖的局部截断误差。在时间信息已知的推理模式下，在扩散-反应基准测试中，我们的方法将外推 RMSE 降低了 87%，同时函数评估次数比 Neural ODE 基准模型少 5 倍。在更具挑战性的直接自回归设置中，即模型必须在没有中间时间线索的情况下预测遥远的未来帧，我们的自适应求解器根据局部几何复杂度分配计算资源——在三个 PDE 基准测试中的两个上保持了最低的外推 RMSE，而基准模型要么发散，要么需要多达一个数量级的函数评估次数才能保持稳定。
