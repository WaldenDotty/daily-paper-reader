...
score: 6.0
evidence: 自主智能体的视觉导航与动作轨迹生成
tldr: 针对具身智能视觉导航中生成式策略推理速度慢的问题，本文提出Rectified Schrödinger Bridge Matching (RSBM) 框架。该框架通过引入熵正则化参数统一了薛定谔桥与确定性最优传输，并证明了速度场结构的不变性。RSBM利用学习到的条件先验缩短传输距离，在减少积分步数的同时保持了多模态覆盖能力，仅需3步积分即可实现高性能导航，显著提升了实时控制效率。
selection_source: fresh_fetch
motivation: 现有的生成式导航策略因高方差随机传输需要大量推理步数，难以满足机器人实时控制的低延迟需求。
method: 提出RSBM框架，利用速度场结构不变性，通过调节熵正则化参数平衡路径直度与多模态覆盖，并结合条件先验优化传输路径。
result: "在无需蒸馏或多阶段训练的情况下，RSBM仅需3步积分即可达到94%的余弦相似度和92%的导航成功率。"
conclusion: RSBM成功缩小了高保真生成策略与具身智能实时性要求之间的差距，为高效视觉导航提供了新方案。
---
