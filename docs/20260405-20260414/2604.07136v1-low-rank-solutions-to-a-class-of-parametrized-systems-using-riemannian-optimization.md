...
## 摘要
我们提出了一种计算框架，用于计算形式为 $A(ξ)x(ξ)+g(x(ξ))=b(ξ)$ 的参数化系统在多个参数值下的解集的低秩逼近。其核心思想是将参数化系统重新解释为定义在实矩阵空间上的优化问题的一阶最优性条件，随后在固定秩矩阵流形上对其进行最小化。这种表述方式使得能够利用黎曼优化技术（包括共轭梯度法和置信域法），并在对参数化系统结构的温和假设下，涵盖了线性和非线性两种情形。我们进一步提供了理论分析，确立了求解矩阵允许精确低秩逼近的条件，将现有结果从线性问题扩展到了非线性问题。为了提高计算效率和鲁棒性，我们讨论了定制的预处理策略以及一种用于控制由非线性引起的秩增长的秩压缩机制。数值实验表明，与独立求解每个系统相比，所提方法显著节省了计算开销，并突显了黎曼优化方法在大规模参数化非线性问题低秩逼近中的潜力。
