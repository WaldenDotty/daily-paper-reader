...
date: 2026-03-17
pdf: "https://arxiv.org/pdf/2603.16982v2"
tags: ["query:cislunar-dyn-nav"]
score: 7.0
evidence: 极端环境下的深空导航与轨迹稳定性
...
method: 开发了一套稳定性理论框架，通过建模喷气驱动校正下的轨道跟踪残差，推导了涵盖扰动能量、收缩性和Floquet稳定性等四个维度的稳定性条件。
result: 确定了高能轨道跟踪的关键约束，并提出了一系列残差诊断指标，如增强的短时误差修正和正则化的标准化创新，用于区分受控与非受控动力学。
conclusion: 该研究为非线性多场扰动下的深空制导控制提供了参考，并为涉及姿态调整或动能撞击的行星防御概念提供了理论支撑。
---

## 摘要
星际天体（ISOs）引发了一个与极端环境下航天器动力学与控制相关的任务设计与推断耦合问题：如果利用富含挥发物、旋转的类彗星天体，通过其既有的双曲运动和原位推进剂进行持续的深空导航，那么在非引力摄动下会产生哪些稳定性要求？哪些天体测量特征可以将主动稳定与无控的自然动力学区分开来？我们开发了一个用于喷气驱动修正轨道跟踪的稳定性理论框架，并表明高速过境几何结构（包括碎片带规避和交会相位调整）严格限制了可行轨道，使得长时程跟踪稳定性成为任务的关键。我们将跟踪残差建模为扰动与修正动作之间的平衡，并推导了四个层面的稳定性条件：扰动能量稳定性、外环收缩性、执行器记忆稳定性和旋转介导（Floquet）稳定性。分析结果暗示了可用于启发实证测试的残差诊断：在同等强度的作用力下，有效的稳定化预计将增强短时程误差修正，减少事件条件下的持续性和方差聚集，使标准化创新正则化，并产生有界的冲击后恢复。更广泛地说，该框架为非线性、多场扰动下的深空制导与控制，以及涉及姿态塑造或脉冲动能撞击的行星防御概念提供了参考。
