...
score: 7.0
evidence: 学习稳定受控动力学系统用于自主导航，处理时变输入
tldr: 现有神经微分方程模型在处理时变控制输入时缺乏严格稳定性保证，或只能处理恒定/消失输入。本文提出输入收缩神经微分模型(ICNDM)，通过输入依赖收缩正则化同时学习非自治神经向量场和广义黎曼收缩度量，实现了增量指数收敛和输入到状态收缩性。在混沌振荡器和永磁同步电机系统上，ICNDM大幅降低长程预测误差，且对输入扰动具有更强鲁棒性。该方法为数据驱动控制提供了结构化的稳定连续时间表示。
source: arxiv
...
motivation: 现有神经ODE无法在时变控制输入下提供严格的稳定性保证，制约了其在动态系统控制中的应用。
method: 提出ICNDM，包含输入编码器和参数化度量网络，通过输入依赖收缩正则化联合学习向量场与收缩度量。
result: 在混沌振荡器和PMSM系统上，ICNDM的长程误差降低显著，且抗输入扰动鲁棒性优于现有神经微分模型。
conclusion: ICNDM实现了时变输入下的收缩特性，为稳定受控动力学系统学习提供了有效框架。
---

## 摘要
从观测数据中学习动力系统的连续时间表示已成为数据驱动控制和科学机器学习的基石。然而，现有的神经微分方程要么启发式地处理外部控制输入而不提供严格的结构保证，要么在恒定或消失输入的 restrictive 假设下强制执行稳定性属性。本文提出了输入收缩神经微分模型（ICNDM），这是一种新颖的深度学习框架，它无缝地整合了时变控制输入，同时通过输入依赖的收缩正则化确保增量指数收敛。通过利用嵌入式输入编码器和参数化度量网络，所提出的架构同时学习非自治神经向量场和广义黎曼收缩度量。我们推导了输入依赖收缩的充分条件，并正式建立了在有界外部激励下的输入到状态收缩性质。对高度非线性混沌振荡器和永磁同步电机（PMSM）驱动系统实验数据的广泛数值评估表明，与最先进的神经微分基准相比，ICNDM 在长时域 rollout 误差上实现了显著降低，并展现出对输入扰动的优越结构鲁棒性。
