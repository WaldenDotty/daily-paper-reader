...
title_zh: 结构模型不确定性下的几何一致性贝叶斯滤波：一种几何投影粒子滤波器
authors: "Surya Ratna Prakash D, Soumyendu Raha"
date: 2026-07-20
pdf: "https://arxiv.org/pdf/2607.17781v2"
tags: ["query:cislunar-dyn-nav"]
score: 7.0
evidence: 几何一致性贝叶斯滤波方法可用于地月空间近距操作的自主导航
tldr: 结构模型不确定性下非线性状态估计面临传播与校正分离导致的粒子退化。本文提出几何一致贝叶斯滤波，将测量几何融入传播过程，通过投影到测量一致子空间并引入测度变换保持后验。开发的几何投影粒子滤波器(GPF)利用几何共态量化动态-测量不一致性，在月球下降导航实验中有效样本量更高、估计误差更低。该框架为鲁棒非线性贝叶斯滤波提供了理论基础。
source: arxiv
...
motivation: 常规贝叶斯滤波分离传播与校正，模型失配积累导致粒子退化和估计精度下降。
method: 提出几何一致贝叶斯滤波，测量几何融入传播，投影到一致子空间，测度变换保持后验，开发GPF。
result: 在月球下降导航实验中，GPF相比传统方法有效样本量更高、估计误差更低。
conclusion: 几何一致传播为鲁棒非线性贝叶斯滤波提供了理论框架。
---

## 摘要
结构模型不确定性下的非线性状态估计仍然是自主制导、导航与控制（GNC）系统中的基本挑战。传统贝叶斯滤波将状态传播与量测校正分离，导致模型失配在传播过程中累积，引发提议分布与似然函数不一致、粒子退化和估计精度下降。现有方法主要改进提议分布或加权策略，而未将量测几何显式融入状态传播。本文提出一种几何一致性贝叶斯滤波框架，将量测几何直接融入传播过程。通过将名义漂移投影到量测一致子空间，得到几何一致性提议分布，同时借助严格的测度变换保持贝叶斯后验。开发了几何投影粒子滤波器（GPF）及一个几何共态，用于量化瞬时动力学与量测之间的不一致性。理论分析证明了投影动力学的存在唯一性、后验保持性、粒子近似的标准蒙特卡洛收敛性以及结构模型不确定性下的鲁棒性。利用部分可观测且存在持续模型不确定性的月球下降导航场景对该框架进行了验证。与自举粒子滤波器和传统高斯滤波方法相比，GPF始终获得更高的有效样本量和更低的估计误差。这些结果表明，几何一致性传播为鲁棒非线性贝叶斯滤波提供了一种原则性、计算高效且具有理论基础的框架。
