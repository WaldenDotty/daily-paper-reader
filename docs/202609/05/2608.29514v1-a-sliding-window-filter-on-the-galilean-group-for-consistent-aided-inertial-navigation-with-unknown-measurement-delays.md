...
score: 7.0
evidence: 面向辅助惯性导航的未知延迟滑动窗口滤波，可直接迁移到地月空间多传感器定位授时融合
tldr: 辅助惯性导航中，传感器测量的未知恒定延迟会破坏状态估计一致性。本文在特殊伽利略群上联合建模运动与时序，发现单次延迟测量存在对称性，单独处理会使信息沿零方向泄漏，导致过度自信。为此提出滑动窗口滤波器，在窗口内联合应用多个时刻的延迟测量以恢复可观测性。仿真表明，即使较短窗口也能显著提升一致性，为未知延迟辅助导航提供可靠估计方案。
source: arxiv
...
motivation: 未知恒定延迟导致延迟与导航状态存在不可观测对称性，单独测量处理会泄漏虚假信息，需联合估计以保证一致性。
method: 在特殊伽利略群上建立导航与延迟的联合模型，通过可观测性分析揭示零方向泄漏，提出保留短历史并在活动窗口内联合应用延迟校正的滑动窗口滤波器。
result: 仿真显示，未保持足够窗口时估计器会迅速高度不一致，即使短滑动窗口也能通过时间支持显著提升一致性。
conclusion: 联合时空建模与滑动窗口机制提供了处理未知延迟的可靠框架，可有效避免信息泄漏，保证辅助惯性导航状态估计的一致性。
---

## 摘要
我们研究了当辅助传感器测量受到未知恒定延迟影响时的辅助惯性导航问题。目标是在联合估计延迟与导航状态的前提下，使延迟测量在适当的时间校正轨迹，从而获得更准确的导航解。我们在特殊伽利略群上构建问题模型，该群为运动与时间均存在不确定性的辅助导航提供了自然的状态空间结构。随后，我们研究了延迟与状态联合估计的可观测性，并表明对于单个延迟测量，模型存在一种精确对称性，其中延迟的改变可以通过导航状态的改变来补偿，而测量保持不变。逐个处理测量可能允许虚假信息沿测量雅可比矩阵对应的零方向“泄漏”，从而产生过于自信且不一致的估计。当轨迹信息充分足够时，同时使用多个时刻的测量可以消除该方向。受此结果启发，我们开发了一种滑窗滤波器，该滤波器保留一段较短的导航状态历史，并在活动窗口内联合应用延迟辅助校正。我们进行了一系列仿真研究，以表征估计器的精度与一致性。仿真表明，未能保持足够窗口的估计器可能迅速变得高度不一致，而即使较短的滑窗也能通过提供可观测性所需的时间支持，显著提高一致性。
