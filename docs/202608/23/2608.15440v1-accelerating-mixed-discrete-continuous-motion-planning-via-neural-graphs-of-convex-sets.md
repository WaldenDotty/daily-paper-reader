...
tldr: "运动规划常需耦合离散决策与连续轨迹，GCS框架虽能建模但求解代价高，难用于在线重规划。本文提出用图注意力网络一次前向预测高概率候选路径，再经轻量排序网络按估计代价排序，按序评估并提前终止，从而避免昂贵凸松弛。在四旋翼、七自由度机械臂和平面推挤任务中，相对名义GCS取得两个数量级加速，成功率保持100%，仅略增子优性。"
source: arxiv
...
motivation: GCS框架解决混合离散-连续运动规划时计算代价高，阻碍在线重规划，需要加速方法。
method: 用图注意力网络预测候选路径，排序网络按代价排序，按序评估并提前终止，替代昂贵凸松弛。
result: "在多种机器人任务中实现对名义GCS的两个数量级加速，成功率100%，但解略有子优。"
conclusion: 提出一种学习加速GCS的通用方案，兼顾速度与成功率，可用于实时运动规划。
---

## 摘要
运动规划问题，如无碰撞导航和接触丰富的操作，可以自然地表述为将离散决策与连续轨迹耦合的优化问题。凸集图（GCS）框架为这些问题提供了一种实用的解决方案。它将离散决策表示为图的节点，并在连接它们的边中编码连续轨迹。然而，由此产生的优化子问题在在线重新规划时可能在计算上变得不可行。在这项工作中，我们提出了一种基于学习的策略来缓解这一限制。具体来说，我们用通过图注意力网络的单次前向传递来替换名义GCS所需的昂贵凸松弛步骤，该网络预测通过图的一组高概率候选路径。然后，一个轻量级排序网络根据其估计的轨迹成本对这些候选路径进行排序。按此顺序评估它们，我们提前终止搜索，同时仍然恢复接近最优的运动计划。我们在多种机器人任务中验证了所提出的流程，包括3D四旋翼和7自由度机械臂的无碰撞运动规划，以及平面推挤的接触规划。在凸和非凸成本及约束设置中，我们的方法相比名义GCS实现了高达两个数量级的加速，同时保持100%的成功率，代价是恢复的解决方案存在一定的次优性。代码实现和视频演示可在https://neural-gcs.github.io/找到。
